...
/**
 * Parse the basic Tokens of the provided text and return them in an array.
 */
export function parseBasicTokens(text: string, startIndex: number = 0): qub.Iterable<qub.Lex> {
...
suite("Qub", () => {
    ...
    suite("ArrayList", () => {
        ...
        suite("foreach()", () => {
            function foreachTest(initialValues: number[]): void {
                test(`with ${JSON.stringify(initialValues)}`, () => {
                    ...
                    arrayList.foreach((element: number) => { sum += element; });
                    assert.deepStrictEqual(sum, expectedSum);
                });
            }
    ...
    suite("Iterable<T>", () => {
        ...
        suite("where()", () => {
            ...
            test("foreach()", () => {
                let values: number[] = [];
                whereIterable.foreach((value: number) => values.push(value));
        ...
        suite("map()", () => {
            ...
            test("foreach()", () => {
                ...
                mapIterable.foreach((value: string) => aggregate += value);
                assert.deepStrictEqual(aggregate, "0123");
            });
    ...
    suite("Iterator<T>", () => {
        ...
        suite("foreach()", () => {
            function foreachTest(values: string[]): void {
                const arrayList = values ? new qub.ArrayList<string>(values) : new qub.ArrayList<string>();
                const i1: qub.Iterator<string> = arrayList.iterate();

                i1.foreach(null);

                let joinedString1: string = "";
                i1.foreach((value: string) => { joinedString1 += value; });
                assert.deepStrictEqual(joinedString1, qub.getCombinedText(values));

                const i2: qub.Iterator<string> = arrayList.iterate();
                i2.next();

                let joinedString2: string = "";
                i2.foreach((value: string) => { joinedString2 += value; });
                assert.deepStrictEqual(joinedString2, qub.getCombinedText(values));
            }
            ...
            test(`with ["a", "b", "c"] and stop()`, () => {
                const arrayList = new qub.ArrayList(["a", "b", "c"]);
                const iterator: qub.Iterator<string> = arrayList.iterate();

                let joinedString: string = "";
                iterator.foreach((value: string, control: qub.Control) => {
                    joinedString += value;
                    if (value === "b") {
                        control.break();
                    }
    ...
    suite("Completion", () => {
        suite("constructor(string)", () => {
            test("with no description argument", () => {
                const completion = new qub.Completion("LABEL", new qub.Span(3, 2));
                assert.deepStrictEqual(completion.label, "LABEL");
                assert.deepStrictEqual(completion.span, new qub.Span(3, 2));
                assert.deepStrictEqual(completion.description, "");
            });

            function constructorTest(label: string, description: string = ""): void {
                test(`with ${qub.quote(qub.escape(label))}`, () => {
                    const completion = new qub.Completion(label, new qub.Span(3, 2), description);
                    assert.deepStrictEqual(completion.label, label);
                    assert.deepStrictEqual(completion.description, description);
                });
            }
            ...
            constructorTest("");
            constructorTest("a");
            constructorTest("a", null);
            constructorTest("a", undefined);
            constructorTest("a", "");
            constructorTest("a", "b");
        });
    });
    ...
    suite("Hover", () => {
        suite("constructor(string[],Span)", () => {
            function constructorTest(textLines: string[], span: qub.Span): void {
                let joinedTextLines: string = "";
                if (textLines) {
                    for (const hoverText of textLines) {
                        if (joinedTextLines) {
                            joinedTextLines += ",";
                        }
                        joinedTextLines = qub.quote(qub.escape(hoverText));
                    }
                }
                joinedTextLines = "[" + joinedTextLines + "]";

                test(`with ${joinedTextLines} and ${span}`, () => {
                    const hover = new qub.Hover(textLines, span);
                    assert.deepStrictEqual(hover.textLines, textLines);
                    assert.deepStrictEqual(hover.span, span);
                });
            }
            ...
            constructorTest([""], null);
            constructorTest(["a"], null);
            constructorTest(["a", "b"], new qub.Span(1, 2));
        });
    });
    ...
    suite("TelemetryEvent", () => {
        test("with no properties", () => {
            const event: qub.TelemetryEvent = {
                eventName: "Abc123"
            };
            assert.deepStrictEqual(qub.telemetryEventToString(event), `"eventName": "Abc123"`);
        });

        test("with one boolean property", () => {
            const event: qub.TelemetryEvent = {
                eventName: "Abc123",
                yes: true
            };
            assert.deepStrictEqual(qub.telemetryEventToString(event), `"eventName": "Abc123", "yes": true`);
        });
        ...
        test("with one string property", () => {
            ...
            assert.deepStrictEqual(qub.telemetryEventToString(event), `"eventName": "Abc123", "apples": "good"`);
        });

        test("with one number property", () => {
            const event: qub.TelemetryEvent = {
                eventName: "Abc123",
                value: -20.7
            };
            assert.deepStrictEqual(qub.telemetryEventToString(event), `"eventName": "Abc123", "value": -20.7`);
        });
        ...
        test("with null property", () => {
            ...
            assert.deepStrictEqual(qub.telemetryEventToString(event), `"eventName": "Abc123", "value": null`);
        });
        ...
        test("with undefined property", () => {
            ...
            assert.deepStrictEqual(qub.telemetryEventToString(event), `"eventName": "Abc123", "value": undefined`);
        });
    });

    suite("TelemetryPropertySetter", () => {
        suite("log(Event)", () => {
            function logTest(properties: qub.TelemetryProperties, event: qub.TelemetryEvent, expectedEvent: qub.TelemetryEvent): void {
                test(`with properties: ${JSON.stringify(properties)} and event: ${JSON.stringify(event)}`, () => {
                    const inMemoryEndpoint = new mock.InMemoryTelemetry();
                    const propertySetter = new qub.TelemetryPropertySetter(inMemoryEndpoint, properties);

                    propertySetter.log(event);

                    assert.deepStrictEqual(inMemoryEndpoint.events.getCount(), 1);
                    assert.deepStrictEqual(inMemoryEndpoint.events.first(), expectedEvent);
            ...
            logTest(null,
                ...
                { eventName: "TestEventName" });
            logTest({ a: "A", b: 0, c: true },
                { eventName: "EventName", d: "D" },
                { eventName: "EventName", d: "D", a: "A", b: 0, c: true });
            logTest({ a: "A", b: 0, c: true, e: () => { return false; } },
                { eventName: "EventName", d: "D" },
                { eventName: "EventName", d: "D", e: false, a: "A", b: 0, c: true });
        });
    });

    suite("LanguageExtension", () => {
        const extensionName: string = "qub-plaintext";
        const extensionVersion: string = "1.0.0";
        const languageId: string = "txt";
        ...
        class PlainTextLanguageExtension extends qub.LanguageExtension<PlainTextDocument> {
            constructor(application: mock.Platform) {
                super(extensionName, extensionVersion, languageId, application);
                this.activate();
            }

            public isParsable(textDocument: qub.TextDocument): boolean {
                return textDocument.getLanguageId() === "txt";
            }
        ...
        suite("constructor()", () => {
            test("with no configuration set", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                assert.deepStrictEqual(application.getFileTelemetryEvents().toArray(), []);
                assert.deepStrictEqual(application.getRemoteTelemetryEvents().toArray(), [
                    { eventName: "Activated" }
                ]);
            ...
            test("with configuration set, but no telemetry values", () => {
                const application = new mock.Platform();
                application.setConfiguration(new mock.Configuration());
            ...
            test("with configuration set and telemetry disabled", () => {
                const application = new mock.Platform();
                application.setConfiguration(new mock.Configuration({
                    "qub-plaintext": {
                        "telemetry": {
                ...
                const extension = new PlainTextLanguageExtension(application);
                assert.deepStrictEqual(application.getFileTelemetryEvents().toArray(), [], "Wrong file telemetry events.");
                assert.deepStrictEqual(application.getRemoteTelemetryEvents().toArray(), [], "Wrong remote telemetry events.");
            ...
            test("with configuration set and empty file path and disabled remote telemetry", () => {
                const application = new mock.Platform();
                application.setConfiguration(new mock.Configuration({
                    "qub-plaintext": {
                        "telemetry": {
                ...
                const extension = new PlainTextLanguageExtension(application);
                assert.deepStrictEqual(application.getFileTelemetryEvents().toArray(), [], "Wrong activated file telemetry.");
                assert.deepStrictEqual(application.getRemoteTelemetryEvents().toArray(), [], "Wrong activated remote telemetry.");

                extension.dispose();
                assert.deepStrictEqual(application.getFileTelemetryEvents().toArray(), [], "Wrong deactivated file telemetry.");
                assert.deepStrictEqual(application.getRemoteTelemetryEvents().toArray(), [], "Wrong deactivated remote telemetry.");
            });

            test("with configuration set and non-empty file path and disabled remote telemetry", () => {
                const application = new mock.Platform();
                application.setConfiguration(new mock.Configuration({
                    "qub-plaintext": {
                        "telemetry": {
                            "enable": true,
                            "file": "MOCK_TELEMETRY_FILE",
                            "remote": false
                        }
                    }
                }));
                ...
                assert.deepStrictEqual(application.getFileTelemetryEvents().toArray(), [
                    { eventName: "Activated" }
                ], "Wrong activated file telemetry.");
                assert.deepStrictEqual(application.getRemoteTelemetryEvents().toArray(), [], "Wrong activated remote telemetry.");
                ...
                assert.deepStrictEqual(application.getFileTelemetryEvents().toArray(), [
                    { eventName: "Activated" },
                    { eventName: "Deactivated" }
                ], "Wrong deactivated file telemetry.");
                assert.deepStrictEqual(application.getRemoteTelemetryEvents().toArray(), [], "Wrong deactivated remote telemetry.");
        ...
        test("version()", () => {
            const application = new mock.Platform();
            const extension = new PlainTextLanguageExtension(application);
            assert.deepStrictEqual(extensionVersion, extension.version);
        });

        test("isParsable()", () => {
            const application = new mock.Platform();
            const extension = new PlainTextLanguageExtension(application);
            assert.deepStrictEqual(extension.isParsable(new mock.TextDocument("txt", "MOCK_URI")), true);
        });

        test("parseDocument()", () => {
            const application = new mock.Platform();
            const extension = new PlainTextLanguageExtension(application);
            assert.deepStrictEqual(application.getFileTelemetryEvents().any(), false);

            const plainTextDocument: PlainTextDocument = extension.parseDocument("plain text");
            assert.deepStrictEqual(plainTextDocument, new PlainTextDocument("plain text"));
        });
        ...
        suite("getConfigurationValue()", () => {
            test("with undefined configuration", () => {
                ...
                assert.deepStrictEqual(extension.getConfigurationValue(undefined, true), true);
            });

            test("with undefined propertyPath", () => {
                const application = new mock.Platform();
                application.setConfiguration(new mock.Configuration());
            ...
            test("with null propertyPath", () => {
                const application = new mock.Platform();
                application.setConfiguration(new mock.Configuration());

                const extension = new PlainTextLanguageExtension(application);
                assert.deepStrictEqual(extension.getConfigurationValue(null), undefined);
                assert.deepStrictEqual(extension.getConfigurationValue(null, false), false);
            });

            test("with not found propertyPath", () => {
                const application = new mock.Platform();
                application.setConfiguration(new mock.Configuration());

                const extension = new PlainTextLanguageExtension(application);
                assert.deepStrictEqual(extension.getConfigurationValue("not.found"), undefined);
                assert.deepStrictEqual(extension.getConfigurationValue("not.found", 300), 300);
            });
            ...
            test("with not found propertyPath", () => {
                ...
                const extension = new PlainTextLanguageExtension(application);
                assert.deepStrictEqual(extension.getConfigurationValue<string>("name"), "Dan");
                assert.deepStrictEqual(extension.getConfigurationValue<number>("age"), 2);
                assert.deepStrictEqual(extension.getConfigurationValue<string[]>("foods"), ["pizza", "pasta"]);
            });
        });

        suite("ConfigurationChangedCallback()", () => {
            test("with configuration set to null with no active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setConfiguration(null);
                assert.deepStrictEqual(extension.getConfigurationValue("key", "oops"), "oops");
            });

            test("with configuration set to undefined with no active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setConfiguration(undefined);
                assert.deepStrictEqual(extension.getConfigurationValue("key", "oops"), "oops");
            });

            test("with configuration set with no active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setConfiguration(new mock.Configuration({
                    "qub-plaintext": {
                ...
                assert.deepStrictEqual(extension.getConfigurationValue("key", "oops"), "value");
            });

            test("with configuration set to null with active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("txt", "MOCK_DOCUMENT_URI", "I'm a text document.")))
                application.setConfiguration(null);
                assert.deepStrictEqual(extension.getConfigurationValue("key", "oops"), "oops");
            });

            test("with configuration set to undefined with active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("txt", "MOCK_DOCUMENT_URI", "I'm a text document.")))
                application.setConfiguration(undefined);
            ...
            test("with configuration set with active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("txt", "MOCK_DOCUMENT_URI", "I'm a text document.")))
                application.setConfiguration(new mock.Configuration({
            ...
            test("with configuration set to null with active editor with no document", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(undefined))
                application.setConfiguration(null);
                assert.deepStrictEqual(extension.getConfigurationValue("key", "oops"), "oops");
            });

            test("with configuration set to undefined with active editor with no document", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(undefined))
                application.setConfiguration(undefined);
            ...
            test("with configuration set with active editor with no document", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(undefined))
                application.setConfiguration(new mock.Configuration({
            ...
            test("with configuration set to null with active editor with non-txt document", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("html", "MOCK_DOCUMENT_URI", "I'm a text document.")))
                application.setConfiguration(null);
                assert.deepStrictEqual(extension.getConfigurationValue("key", "oops"), "oops");
            });

            test("with configuration set to undefined with active editor with non-txt document", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("html", "MOCK_DOCUMENT_URI", "I'm a text document.")))
                application.setConfiguration(undefined);
            ...
            test("with configuration set with active editor with non-txt document", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("html", "MOCK_DOCUMENT_URI", "I'm a text document.")))
                application.setConfiguration(new mock.Configuration({
            ...
            test("with configuration set with no active editor after a txt document editor has been active", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("html", "MOCK_DOCUMENT_URI", "I'm a text document.")))
                application.setConfiguration(new mock.Configuration({
                ...
                application.setActiveTextEditor(null);
                assert.deepStrictEqual(extension.getConfigurationValue("key", "oops"), "value");
            });

            test("with configuration set, remote telemetry enabled, but no remote telemetry set", () => {
                const application = new mock.Platform();
                application.setRemoteTelemetry(undefined);
        ...
        suite("setOnParsedDocumentOpened()", () => {
            test("with no callback assigned", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);

                let openedDocuments: number = 0;

                const textDocument = new mock.TextDocument(languageId, "MOCK_URI.txt", "blah blah");
                application.openTextDocument(textDocument);

                assert.deepStrictEqual(openedDocuments, 0);
            });

            test("with callback assigned", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);

                let openedDocuments: number = 0;

                extension.setOnParsedDocumentOpened((parsedDocument: PlainTextDocument) => {
                    ++openedDocuments;
                });
                ...
                assert.deepStrictEqual(openedDocuments, 1);
            });
        });
        ...
        suite("setOnParsedDocumentChanged()", () => {
            test("with no callback assigned", () => {
                ...
                let changedDocuments: number = 0;

                const textDocument = new mock.TextDocument(languageId, "MOCK_URI.txt", "blah blah");
                application.openTextDocument(textDocument);
                application.insertText(0, "hello");
                assert.deepStrictEqual(changedDocuments, 0);
                assert.deepStrictEqual(application.getActiveTextEditor().getDocument().getText(), "helloblah blah");
            });
            ...
            test("with callback assigned", () => {
                ...
                extension.setOnParsedDocumentChanged((parsedDocumentChange: qub.ParsedDocumentChange<PlainTextDocument>) => {
                    ++changedDocuments;
                });
                ...
                application.openTextDocument(textDocument);
                application.insertText(0, "hello");
                assert.deepStrictEqual(changedDocuments, 1);
                assert.deepStrictEqual(application.getActiveTextEditor().getDocument().getText(), "helloblah blah");
            });

            test("with callback assigned but different file type changed", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                ...
                application.openTextDocument(new mock.TextDocument(languageId, "MOCK_URI.txt", "blah blah"));
                application.openTextDocument(new mock.TextDocument("xml", "MOCK_URI.xml", "BLAH BLAH"));
                application.insertText(0, "hello");
                assert.deepStrictEqual(changedDocuments, 0);
                assert.deepStrictEqual(application.getActiveTextEditor().getDocument().getText(), "helloBLAH BLAH");
            });
        ...
        suite("setOnParsedDocumentSaved()", () => {
            test("with no callback assigned", () => {
                ...
                let savedDocuments: number = 0;

                const textDocument = new mock.TextDocument(languageId, "MOCK_URI.txt", "blah blah");
                application.openTextDocument(textDocument);
                application.saveTextDocument(textDocument);
                assert.deepStrictEqual(savedDocuments, 0);
            });
            ...
            test("with callback assigned", () => {
                ...
                extension.setOnParsedDocumentSaved((parsedDocument: PlainTextDocument) => {
                    ++savedDocuments;
                    assert.deepStrictEqual(parsedDocument, new PlainTextDocument("blah blah"));
                });

                const textDocument = new mock.TextDocument(languageId, "MOCK_URI.txt", "blah blah");
                application.openTextDocument(textDocument);
                application.saveTextDocument(textDocument);
                assert.deepStrictEqual(savedDocuments, 1);
            });
        });
        ...
        suite("setOnParsedDocumentClosed()", () => {
            test("with no callback assigned", () => {
                ...
                let closedDocuments: number = 0;

                const textDocument = new mock.TextDocument(languageId, "MOCK_URI.txt", "blah blah");
                application.openTextDocument(textDocument);
                assert.deepStrictEqual(closedDocuments, 0);

                application.closeTextDocument(textDocument);
                assert.deepStrictEqual(closedDocuments, 0);
            });

            test("with callback assigned, but different language file closed", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);

                let closedDocuments: number = 0;
                extension.setOnParsedDocumentClosed((closedDocument: PlainTextDocument) => {
                    ++closedDocuments;
                });

                // This is required to get our onParsedDocumentClosed() event handler enabled.
                application.openTextDocument(new mock.TextDocument(languageId, "MOCK_URI.txt", "blah blah 2"));

                const textDocument = new mock.TextDocument("xml", "MOCK_URI.xml", "blah blah");
                application.openTextDocument(textDocument);
                assert.deepStrictEqual(closedDocuments, 0);
            ...
            test("with callback assigned and plaintext file closed", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                ...
                extension.setOnParsedDocumentClosed((closedDocument: PlainTextDocument) => {
                    ++closedDocuments;
                    assert.deepStrictEqual(closedDocument, new PlainTextDocument("blah blah"));
                });

                extension.setOnProvideIssues((parsedDocument: PlainTextDocument) => new qub.ArrayList<qub.Issue>());

                application.openTextDocument(textDocument);
                assert.deepStrictEqual(closedDocuments, 0);

                application.closeTextDocument(textDocument);
                assert.deepStrictEqual(closedDocuments, 1);
            });
        });

        test("setOnProvideIssues()", () => {
            const application = new mock.Platform();
            const extension = new PlainTextLanguageExtension(application);

            extension.setOnProvideIssues((parsedDocument: PlainTextDocument) => {
                return new qub.ArrayList<qub.Issue>();
            });

            application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("txt", "MOCK_URI", "I'm a text document")));
        });
        ...
        suite("setOnProvideHover()", () => {
            test("with no active editor", () => {
                ...
                extension.setOnProvideHover((parsedDocument: PlainTextDocument, index: number) => {
                    return new qub.Hover(["I'm a hover!"], new qub.Span(index, 12));
                });

                assert.deepStrictEqual(application.getHoverAt(4), undefined);
            });

            test("with active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                ...
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("txt", "MOCK_URI", "I'm not a hover!")));

                assert.deepStrictEqual(application.getHoverAt(4), new qub.Hover(["I'm a hover!"], new qub.Span(4, 12)));
            });

            test("with non-txt active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
                ...
                // First open a .txt file to setup the language specific events.
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("txt", "MOCK_URI1", "I'm not a hover!")));
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("html", "MOCK_URI2", "I'm not a hover!")));
        ...
        suite("setOnProvideCompletions()", () => {
            test("with no active editor", () => {
                ...
                extension.setOnProvideCompletions(["."], (parsedDocument: PlainTextDocument, index: number) => {
                    return new qub.ArrayList([
                        new qub.Completion("I'm a completion!", new qub.Span(3, 2), "With a description")
                    ]);
                });

                assert.deepStrictEqual(application.getCompletionsAt(4).toArray(), []);
            });
            ...
            test("with previous txt active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);
        ...
        suite("setProvideFormattedDocumentText()", () => {
            test("with no active editor", () => {
                const application = new mock.Platform();
                const extension = new PlainTextLanguageExtension(application);

                extension.setOnProvideFormattedDocument((parsedDocument: PlainTextDocument) => {
                    return parsedDocument.text.replace("a", "");
                });

                assert.deepStrictEqual(application.getFormattedDocument(), undefined);
            });
            ...
            test("with active editor", () => {
                ...
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("txt", "MOCK_URI", "I'm a real text document.")));

                assert.deepStrictEqual(application.getFormattedDocument(), "I'm  real text document.");
            });
            ...
            test("with non-txt active editor", () => {
                ...
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("txt", "MOCK_URI", "I'm a real text document.")));
                application.setActiveTextEditor(new mock.TextEditor(new mock.TextDocument("html", "MOCK_URI2", "I'm a real text document.")));
        ...
        suite("isExtensionInstalled()", () => {
            test("when extension is not installed", () => {
                ...
                assert.deepStrictEqual(extension.isExtensionInstalled("stars", "hollow"), false);
            });

            test("when extension is installed", () => {
                const application = new mock.Platform();
                application.addInstalledExtension("stars", "hollow");

                const extension = new PlainTextLanguageExtension(application);

                assert.deepStrictEqual(extension.isExtensionInstalled("stars", "hollow"), true);
            });
        });
        ...
        test("consoleLog()", () => {
            const application = new mock.Platform();
            const extension = new PlainTextLanguageExtension(application);
            extension.consoleLog("hello world!");
            assert.deepStrictEqual(application.getConsoleLogs().toArray(), ["qub-plaintext: hello world!"]);
        });
        ...
        test("consoleTrace()", () => {
            ...
            let x = 0;
            extension.consoleTrace("setX()", () => { x = 20; });
            assert.deepStrictEqual(x, 20);
            assert.deepStrictEqual(application.getConsoleLogs().toArray(), [
                "qub-plaintext: setX() - Enter",
                "qub-plaintext: setX() - Exit"
            ])
        });
    });

    suite("Token", () => {
        suite("constructor(TokenType,number)", () => {
            ...
            function nextTest(text: string, expectedTokens: qub.Lex[]): void {
                test(`with ${qub.quote(qub.escape(text))}`, () => {
                    ...
                    for (const expectedToken of expectedTokens) {
                        assert.deepStrictEqual(tokenizer.next(), true, "Wrong next()");
                        assert.deepStrictEqual(tokenizer.hasStarted(), true, "Wrong hasStarted()");
                        assert.deepStrictEqual(tokenizer.getCurrent(), expectedToken, "Wrong getCurrent()");
                    }

                    for (let i: number = 0; i < 2; ++i) {
                        assert.deepStrictEqual(tokenizer.next(), false);
